...
def test_atomic_save_preserves_original_file_on_replace_failure(workbook_path: Path, monkeypatch):
    original_bytes = workbook_path.read_bytes()

    def _raise(*_args, **_kwargs):
        raise OSError("replace failed")

    monkeypatch.setattr("excel_dbapi.engine.openpyxl_engine.os.replace", _raise)
